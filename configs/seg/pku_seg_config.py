...
NGRAM_DEFS = ["w[k];w[k-1];w[k+1]", "w[k],w[k+1];w[k-1],w[k]", "w[k-2],w[k-1],w[k];w[k],w[k+1],w[k+2];w[k-1],w[k],w[k+1]"]
...
def get_emb(path,words_num,vec_size=2):
    ...
    for line in open(path):
        ...
        emb.append(vals)
    rval = np.array(emb+[[0,]*vec_size,]*(words_num-len(emb)), dtype=np.float32)
    return rval
        
def get_config(branch="develop"):
      
    default_batch_size = 1000
    ngram_words = [{k:w[:-1] for k,w in enumerate(open(path))} for path in NGRAM_PATHS]
    ngram_sizes_ =sum([[len(words)+5,]*len(NGRAM_DEFS[k].split(';')) for k,words in enumerate(ngram_words)],[])
    ngram_feature_vecs = sum([[get_emb(path, len(ngram_words[k])+5),]*len(NGRAM_DEFS[k].split(';')) for k,path in enumerate(NGRAM_FEATURE_PATHS)],[])
    print ngram_sizes_
    ...
    class Config():
        ...
        class model_config():
            ...
            net_crf = [# 这里输入输出的name表示self.name,而不是计算图中的名字
                [("input_zh_x", "input_zh_y_seg", "input_zh_x_length"),    "CRF",     crf_args,  "crf",   ("predictions_zh_seg", "loss_zh_seg")],
                [("loss_zh_seg",),         "TrainOp",        train_args,     "train",   ("train_op_zh_seg","lr3")],
            ]   

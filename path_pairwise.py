...
class PathScore():
  ...
  def apply_by_formula1(self, inputs):
    ...
    cnt_word_alpha = tf.ones_like(cnt)-single_word*(1-self.single_word_ratio)
    ...
    final_weight = tf.reduce_sum(global_weight + self.cut_ratio*cut_weight, -1)
    return final_weight
...
class Model():
  ...
  def __init__(self):
      ...
      self.path_inputs = [self.path_len, self.word_len, self.q_count, self.t_count, self.qt_tight, self.tq_tight, self.qv, 
        self.ngram_next_tight, self.ngram_next_match_count]
      self.path_score_model = PathScore()
      self.final_weight = self.path_score_model.apply(self.path_inputs)
      self.loss = PairwiseLoss(self.final_weight, self.postive_indexs, self.nagative_indexs, self.pair_weight)
    ...
    while True:
      try:
        ...
        for i in range(path_count):
          line = ii.readline()
          edges = line.split('\t')
          edges = [dict([s.split('<=/>') for s in edge.split('<,/>')]) for edge in edges]
          path_attrs = []
          for edge in edges:

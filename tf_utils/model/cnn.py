...
class MultiFilterSizesCnn(object):
    ...
    def apply(self, inputs, debug=False):
        ...
        for filter_size,filter_num in zip(self.filter_sizes, self.filter_nums):
            if self.front_pad!=0: front_pad=filter_size-1
            else: front_pad=0
            if self.padding.lower()=="same" and filter_size>=1:
                inputs_=tf.pad(inputs, [[0,0],[front_pad, filter_size-1-front_pad],[0,0],[0,0]])
                # inputs_2=tf.pad(inputs, [[0,0],[0,filter_size-1],[0,0],[0,0]])
            else:
                inputs_=inputs
            print 'CNN on size', inputs_, filter_size, self.emb_length

...
class Classifier(object):
    ...
    def __init__(self, model_path, use_gpu=False):
        ...
        config=load_config(model_dir+'/ner_config.py', ceph_path="/ceph_ai", branch="develop")
        ...
        self.info = info
        config.seq_len=20
        self.seq_ps ={name: sequence_line_processing(config_.vocab, return_length=True, seq_len=config_.seq_len, split=config_.tok, sub_words=None, char_len=10) for name,config_ in config.data_config.task2configs.iteritems()} 
        print self.tags, self.seq_ps
        self.tf_model=TFModel(sess,model_path, info)

    def predict(self, line, task_name):
        tensors=[np.array(x) for x in self.seq_ps[task_name](self.pre(line))]+[1.0]
        ...
        if type=="str":
            ...
            for line,rval in zip(lines, rvals):
                ...
                for state, word, loc in rval[::-1]:
                    line=line[:loc[0]]+' '+word+'/'+state+' '+line[loc[1]:]
    ...
    def get_ner_(self, text, task_name):
        ...
        while text1 and (text1[-1] >="a" and text1[-1]<="z" or text1[-1]=="'"):
            text1=text1[:-1]
        if not text1.strip(): return []
        ...
        for k,(ch,tag) in enumerate(zip(text1, labels)):
            ...
            if tag=='O' or tag=='</pad>' or tag=='</s>' or ch in ['\n']:
                if state and word:
                    rval.append((state,word,(start,start+len(word.encode('utf-8')))))
                state,word=None,''
            else:
                l,state_ = tag.split('-')
                if state:
        ...
        if state and word:
            rval.append((state,word,(start,start+len(word.encode('utf-8')))))
        rval=[(state, word.strip().encode('utf-8'), loc) for state,word,loc in rval if word.strip() and ' ' not in word and not bad_ner.match(word)]
        for i,(state, word, loc) in enumerate(rval):
            if '京东' in word and word!='京东':
                rval[i]=(state, '京东', (loc[0]+word.index("京东"), loc[0]+word.index("京东")+6))
        return rval
...
if __name__=="__main__":
    ...
    if args.method=="demo":
        while True:
            ...
            print 'RESULT=', json.dumps(tag_model.get_ner(line, "seg"), ensure_ascii=False)
            print 'RESULT=', json.dumps(tag_model.get_ner(line, "pos"), ensure_ascii=False)
            print 'RESULT=', json.dumps(tag_model.get_ner(line, "ner"), ensure_ascii=False)

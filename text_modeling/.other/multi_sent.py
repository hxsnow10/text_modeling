...
def load_data(config, mode="train"):
    class Data():
        ...
        def load_words(path, skip_head, max_vocab_size):
            ...
            if skip_head:
                ff.readline()
            words = {k: word.split()[0] for k, word in enumerate(
                islice(ff, max_vocab_size)) if k < max_vocab_size or max_vocab_size is None}
            return words
        print config.words_vec.vocab_path
        words = config.words_vec and load_words(
            config.words_vec.vocab_path,
            config.words_vec.vocab_skip_head,
            config.words_vec.max_vocab_size)
        sub_words = config.chars_vec and load_words(
            config.chars_vec.vocab_path,
            config.chars_vec.vocab_skip_head,
            config.chars_vec.max_vocab_size)
        ...
        if mode == 'train':
            ...
            train_data = LineBasedDataset(
                config.train_paths,
                line_processing=line_p,
                batch_size=config.batch_size)
            dev_data = LineBasedDataset(
                config.dev_paths,
                line_processing=line_p,
                batch_size=config.batch_size)

...
class FSMN(object):
    ...
    def build_graph(self):
        self._W1 = tf.get_variable(
            "fsmnn_w1", [
                self._input_size, self._output_size], initializer=tf.truncated_normal_initializer(
                stddev=5e-2, dtype=self._dtype))
        self._W2 = tf.get_variable(
            "fsmnn_w2", [
                self._input_size, self._output_size], initializer=tf.truncated_normal_initializer(
                stddev=5e-2, dtype=self._dtype))
        self._bias = tf.get_variable(
            "fsmnn_bias", [
                self._output_size], initializer=tf.constant_initializer(
                0.0, dtype=self._dtype))
        self._memory_weights = tf.get_variable(
            "memory_weights", [
                self._memory_size], initializer=tf.constant_initializer(
                1.0, dtype=self._dtype))
    ...
    def __call__(self, input_data):
        ...
        cc = time.time()
        h_hatt = tf.matmul(
            [memory_matrix] * batch_size,
            input_data)  # batch_size*200*128
        # bs*200*128  X  bs*128*128  =  bs*200*128
        h = tf.matmul(input_data, [self._W1] * batch_size)
        h += tf.matmul(h_hatt, [self._W2] * batch_size) + \
            self._bias  # bs*200*128
        dd = time.time()
...
class BiFSMN(object):
    ...
    def build_graph(self):
        ...
        self._bias = tf.get_variable(
            "fsmnn_bias", [
                self._output_size], initializer=tf.constant_initializer(
                0.0, dtype=self._dtype))
        self._memory_weights_a = tf.get_variable(
            "memory_weights_a", [
                self._memory_size_a], initializer=tf.constant_initializer(
                1.0, dtype=self._dtype))
        self._memory_weights_c = tf.get_variable(
            "memory_weights_c", [
                self._memory_size_c], initializer=tf.constant_initializer(
                1.0, dtype=self._dtype))
    ...
    def __call__(self, input_data):
        ...
        for step in range(num_steps):
            ...
            right_num = tf.maximum(
                0, num_steps - step - 1 - self._memory_size_c)
            mem_ahead = self._memory_weights_a[tf.minimum(
                step, self._memory_size_a)::-1]  # look-ahead
            mem_back = self._memory_weights_c[:tf.minimum(
                num_steps - step - 1, self._memory_size_c)]  # look-back
...
class AttbBiFSMN(object):
    ...
    def build_graph(self):
        ...
        # self._V = tf.Variable(self.xaiver_init(1, self._memory_size))#v4 ->
        # v7
        self._V = tf.Variable(
            self.xaiver_init(
                self._memory_size,
        ...
        # self._m = tf.get_variable("attention_m",
        # [1],initializer=tf.truncated_normal_initializer(stddev=5e-2,
        # dtype=self._dtype))#v4 -> v7
        self._m = tf.Variable(
            self.xaiver_init(
                self._seq_len,
                1))  # scalar -> vector #v5 -> v6

    def xaiver_init(self, d1, d2, constant=1):
        low = -constant * np.sqrt(6.0 / (d1 + d2))
        high = constant * np.sqrt(6.0 / (d1 + d2))
        return tf.random_uniform(
            (d1, d2), minval=low, maxval=high, dtype=tf.float32)
    ...
    def __call__(self, input_data):
        ...
        # v5-> v6 ------start
        self._memory_weights = tf.matmul(
            [self._V] * batch_size, tf.tanh(tf.matmul(input_data, [self._U] * batch_size) + self._m))
        ...
        for i in range(batch_size):
            ...
            memory_matrix = tf.zeros(
                [num_steps, num_steps + self._memory_size - 1])
            ...
            memory_matrix = memory_matrix[:,
                                          self._memory_size_a - 1:-self._memory_size_c]
            # memory_matrix = []
            # for step in range(num_steps):

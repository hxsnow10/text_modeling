...
every subnet(word2vec, cnn, rnn, outputs, loss, etc)
    __init__ with net super-params;
    __apply__ at input-tensor to produce output-tensor.
all subnets are combined to a map,
config define network by net-name and superparams
    without really import.
"""
...
class Word2Vec(SubNet):
    ...
    def __init__(
        ...
        for arg in func_args:
            setattr(self, arg, func_args[arg])
    ...
    def __call__(self, input_x, input_x_sub=None):
        ...
        def build_emb(init_emb, name, shape=None):
            ...
            if init_emb is not None:
                ...
                weight = tf.get_variable(
                    name, initializer=init_emb, trainable=False)
            else:
                weight = tf.get_variable(
                    name,
                    shape=shape,
        ...
        word_weight = build_emb(self.init_emb, "word_W", self.w2v_shape)
        words_vec = tf.cast(
            tf.nn.embedding_lookup(
                word_weight, input_x), tf.float32)
        ...
        if self.sub_init_emb is not None or self.sub_w2v_shape is not None:
            ...
            chars_vec = tf.cast(
                tf.nn.embedding_lookup(
                    char_weight, input_x_sub), tf.float32)
            chars_vec = tf.reshape(
                chars_vec, [
                    batch_size * seq_len, char_len, -1])
            vec_size_c = self.sub_init_emb.shape[-1]
            word_vec = multi_filter_sizes_cnn(
                tf.expand_dims(
                    chars_vec,
                    -1),
                10,
                vec_size_c,
                self.sub_cnn.char_filter_sizes,
                self.sub_cnn.char_filter_nums,
                name='char_cnn',
                reuse=False)
...
class Cnn(SubNet):
    ...
    def __call_(self, words_vec):
        ...
        seq_len, vec_size =\
            shape_x[1], shape_x[2]  # noqa: F841
        if self.debug:
            self.sent_vec, self.pooled_index = multi_filter_sizes_cnn_debug(
                words_vec, self.seq_len, self.vec_size,
                self.filter_sizes, self.filter_nums,
                name='cnn', reuse=self.reuse)
        else:
            for i in range(self.cnn_layer_num):
                words_vec = tf.expand_dims(words_vec, -1)
                words_vec_1 = multi_filter_sizes_cnn(
                    words_vec,
                    seq_len,
                    vec_size,
                    self.filter_sizes,
                    ...
                    padding="same",
                    front_pad='MAX')
                words_vec_2 = multi_filter_sizes_cnn(
                    words_vec,
                    seq_len,
                    vec_size,
                    self.filter_sizes,
                    ...
                    front_pad=0)
                words_vec = tf.concat(
                    [words_vec_1, words_vec_2], 2) if self.bi else words_vec_1
                vec_size = 2 * \
                    sum(self.filter_nums) if self.bi else sum(self.filter_nums)
        return words_vec
    ...
    def build_sent_add(self):
        self.mask = tf.cast(
            tf.sequence_mask(
                ...
                config.seq_len),
            tf.float32)
        self.sent_vec = tf.reduce_sum(tf.expand_dims(
            self.mask, -1) * self.words_emb[:, :, :, 0], 1)
...
class SumConcat(SubNet):
    ...
    def __call__(self, words_vec, input_sequence_length, dropout=None):
        ...
        def move(words_vec, i):
            if i > 0:
                return tf.pad(words_vec[:, i:], [[0, 0], [0, i], [0, 0]])
            else:
                return tf.pad(words_vec[:, :i], [[0, 0], [-i, 0], [0, 0]])
        words_vec_p1 = move(words_vec, 1)
        words_vec_p2 = move(words_vec, 2)
        words_vec_n1 = move(words_vec, -1)
        words_vec_n2 = move(words_vec, -2)
        ...
        words_vec = tf.concat(
            [words_vec_0, words_vec_1, words_vec_2, words_vec_3, words_vec_4], 2)
...
'''
    def build_cnn_rnn(self):
        words_emb= multi_filter_sizes_cnn(
            self.words_emb, self.seq_len, self.vec_size,
            config.filter_sizes, config.filter_nums,
            name='cnn', reuse=False, pooling=False, padding="same")
        words_emb=rnn_func(words_emb, config.rnn_cell,
            config.cell_size, config.rnn_layer_num,
            sequence_length=self.input_sequence_length,
            attn_type=config.attn_type, bi=config.bi)
...
class ExclusiveTagOutputs(SubNet):
    """outputs layer for classification"""

    def __init__(self, debug, l2_lambda, class_weights):
        func_args = get_func_args()
        for arg in func_args:
            setattr(self, arg, func_args[arg])
        super(ExclusiveTagOutputs, self).__init__()
    ...
    def __call__(self, inputs, input_y):
        ...
        with tf.name_scope("loss"):
            ...
            losses = tf.nn.softmax_cross_entropy_with_logits(
                labels=input_y, logits=scores)
            if self.class_weights:
                # 这里做了个糟糕的假设，假设input_y[i]只有一个是1，其余是0
...
class NoExclusiveTagOutputs(SubNet):
    """outputs layer for milticlass tagging"""

    def __init__(self, ):
        func_args = get_func_args()
        for arg in func_args:
            setattr(self, arg, func_args[arg])
        super(NoExclusiveTagOutputs, self).__init__()
    ...
    def __call__(self, input_y):
        ...
        with tf.name_scope("output"):
            with tf.variable_scope("share"):
                scores = tf.layers.dense(
                    ...
                    name="dense",
                    reuse=self.reuse)
            scores = tf.reshape(scores, [self.batch_size, self.num_classes, 2])
            predictions = tf.argmax(scores, -1, name="predictions")
        ...
        with tf.name_scope("accuracy"):
            ...
            self.accuracy = tf.reduce_mean(tf.cast(correct_predictions, "float"),
                name="accuracy")
            self.class_accuracy = tf.reduce_mean(tf.cast(correct_predictions, "float"),
                axis = 0, name="class_accuracy")
        '''
        return predictions, loss
...
'''
    def build_weighted_sampled_outputs(self, inputs, num_classes):
        with tf.name_scope("output"):
            weights = tf.get_variable("weights",shape=[num_classes, self.repr_size],
                        dtype=tf.float32)
        ...
        with tf.name_scope("loss"):
            ...
            out_logits, out_labels=\
                _compute_sampled_logits( weights, biases, self.input_y, inputs,\
    ...
    def build_sampled_outputs(self, inputs, num_classes, exclusive):
        ...
        if config.tag_exclusive:
            losses=tf.nn.sampled_softmax_loss(
                weights, biases, self.input_y, inputs,
                config.num_sampled, num_classes, num_true=1,
                partition_strategy="div")
        else:
            losses=tf.nn.nec_loss(
                weights, biases, self.input_y, inputs,
                config.num_sampled, num_classes, num_true=1,
                partition_strategy="div")
...
class CRF(SubNet):
    ...
    def __init__(self, ngram_sizes=None, tag_size=None, init_embs=None):
        ngram_sizes = [] or ngram_sizes
        init_embs = [] or init_embs
        func_args = get_func_args()
        for arg in func_args:
            setattr(self, arg, func_args[arg])
    ...
    def __call__(self, input_x, input_y, input_sequence_length):
        ...
        weight1 = [
            build_emb(
                None, "prob_{}".format(i), [
                    ngram_size, self.tag_size], True) for i, ngram_size in enumerate(
                self.ngram_sizes)]
        scoress1 = [tf.nn.embedding_lookup(
            weight1[i], input_x[:, :, i]) for i in range(len(weight1))]
        ...
        weight2 = [
            build_emb(
                ...
                None,
                False) for i,
            ngram_size in enumerate(
                self.ngram_sizes)]
        scoress2 = [tf.layers.dense(tf.nn.embedding_lookup(
            weight2[i], input_x[:, :, i]), self.tag_size, name="linear_map_{}".format(i))
            for i in range(len(weight2))]
        # scoress2 = [
        #   tf.layers.dense(scoress2[i], self.tag_size,
        #   name="linear_map_2_{}".format(i), activation=tf.nn.relu)
        # for i in range(len(scoress2))]
...
def Outputs(objects, num_classes, *args, **kwargs):  # pylint: disable=invalid-name
    """combined outputs module
    """
    if objects == "tag":
        # TODO
        if None:
            pass
            #  self.build_sampled_outputs(
            #    self.sent_vec,
            #    self.num_classes,
            #    config.tag_exclusive)
        elif "tag_exclusive":
            return ExclusiveTagOutputs(num_classes, *args, **kwargs)
        else:
            return NoExclusiveTagOutputs(num_classes, *args, **kwargs)

...
class Classifier(object):
    ...
    def __init__(self, model_path, use_gpu=False):
        self.pos_norm = {line.strip().split('\t')[0]: line.strip().split('\t')[
            1] for line in open("/ceph_ai/xiahong/data/segment_corpus/pos_tokens/pos_norm.txt")}
        print self.pos_norm
        model_dir = os.path.dirname(model_path)
        config = load_config(
            model_dir + '/pos_config.py',
            ceph_path="/ceph_ai",
            branch="develop")
        ...
        self.seq_ps = {
            name: sequence_line_processing(
                ...
                split=' ',
                sub_words=config_.sub_vocab,
                char_len=10,
                use_char_length=False) for name,

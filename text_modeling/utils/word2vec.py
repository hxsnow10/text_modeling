...
def load_w2v(
    ...
    for k, line in enumerate(ii):
        try:
            wl = line[:-1].split()
            word = wl[0]
            if limited_words and word not in limited_words:
                continue
            ...
            print line
        if max_vocab_size and k >= max_vocab_size:
            break
...
class getw2v(object):
    def __init__(
        ...
        if vec_path:
            ...
            self.init_emb = np.array(w2v.values(
            ) + [[0, ] * self.vec_size, ] * (len(self.vocab) - len(w2v)), dtype=np.float32)
        else:
            self.init_emb = np.random.random(
                [len(self.vocab), self.vec_size]) * 0.01

...
def evaluate_clf(sess, model, eval_data, target_names=None, restore=False, dropout=True):
...
def evaluate_tag(sess, model, eval_data, target_names=None, restore=False, dropout=True):
    total_y,total_predict_y = [], []
    print 'start evaluate...'
    for inputs in eval_data:
        if dropout:
...
def evaluate_seq_tag(sess, model, task_name, eval_data, target_names=None, restore=False, dropout=True):
    ...
    def get_seq_for_segment(y, ignore=['</s>', '</pad>', 'O']):
        y = y.tolist()
        if not y: return []
        def get_tag(x):
            x = target_names[x] 
    ...
    for k,inputs in enumerate(eval_data):
        if dropout:
            inputs["dropout"]=1.0
        fd = {getattr(model, name):inputs[name] for name in model.predict_task2io[task_name]["inputs"].keys()}
    ...
    return seq_f,{"precision":p,"recall":r,"f1":f,"seq_p":seq_p, "seq_r":seq_r, "seq_f1": (2*seq_p*seq_r)/(seq_p+seq_r+0.0001)}
